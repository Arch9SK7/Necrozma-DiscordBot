...
class Blah:
    ...
    async def announce(self, ctx, *, inp):
        ...
        await self.bot.send_message(discord.utils.get(server.channels, name="news"), inp)

    @commands.has_permissions(ban_members=True)
    @commands.command(hidden=True, pass_context=True)
    async def pannounce(self, ctx, *, inp):
        server = ctx.message.server
        await self.bot.send_message(discord.utils.get(server.channels, name="pokenews"), inp)
