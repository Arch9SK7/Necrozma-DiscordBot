...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Necrozma - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Necrozma",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Necrozma in case of issues",
                        action="store_true")
    parser.add_argument("--update-necrozma",
                        help="Updates Necrozma (git)",
                        action="store_true")
...
def update_necrozma():
    try:
        ...
        return
    if code == 0:
        print("\nNecrozma has been updated")
    else:
        print("\nNecrozma could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")


def reset_necrozma(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if git_reset:
        ...
        if code == 0:
            print("Necrozma has been restored to the last local commit.")
        else:
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Necrozma:")
        print("1. Update Necrozma + requirements (recommended)")
        print("2. Update Necrozma")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Necrozma (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_necrozma(data=True)
                wait()
        elif choice == "3":
            reset_necrozma(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your Necrozma's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back.")
            if user_pick_yes_no():
                reset_necrozma(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_necrozma(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Necrozma. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "necrozma.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Necrozma...")
                continue
            else:
                if not autorestart:
                    break

    print("Necrozma has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Necrozma without going
    ...
    start_necrozma = "{} --start".format(call)
    start_necrozma_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_necrozma             = ccd + start_necrozma             + pause
    start_necrozma_autorestart = ccd + start_necrozma_autorestart + pause

    files = {
        "start_necrozma"             + ext : start_necrozma,
        "start_necrozma_autorestart" + ext : start_necrozma_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Necrozma Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Necrozma has been "
        ...
        print("1. Run Necrozma /w autorestart in case of issues")
        print("2. Run Necrozma")
        print("3. Update")
        ...
        choice = user_choice()
        if choice == "1":
            run_necrozma(autorestart=True)
        elif choice == "2":
            run_necrozma(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Necrozma needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Necrozma cannot work without the pip module. Please make sure to "
              "install Python without unchecking any option during the setup")
        wait()
        exit(1)
    if args.repair:
        reset_necrozma(git_reset=True)
    if args.update_necrozma:
        update_necrozma()
    if args.update_reqs:
        install_reqs(audio=True)
    ...
    if INTERACTIVE_MODE:
        ...
        print("Starting Necrozma...")
        run_necrozma(autorestart=args.auto_restart)
